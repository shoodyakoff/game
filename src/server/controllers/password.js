...
const forgotPassword = async (req, res) => {
  try {
    ...
    const resetUrl = `${process.env.NEXT_PUBLIC_APP_URL}/auth/forgot-password/reset?token=${resetToken}`;
    ...
    res.status(200).json({
      ...
      data: {
        resetUrl // В реальном приложении это не нужно возвращать клиенту
      }
    });
  } catch (error) {
