...
## Предотвращение ошибок типизации и стилей

1. **Проверка существования стилей**:
   - Перед использованием стиля `styles.someName` убедитесь, что этот стиль определен в соответствующем файле стилей
   - Используйте автодополнение IDE при обращении к стилям для проверки их существования
   - Запрещено использовать стили, которые не определены в файле стилей

2. **Согласованность стилей**:
   - При добавлении нового компонента сначала изучите существующие стили и переиспользуйте их
   - Для новых UI-элементов проверьте, не существует ли аналогичный стиль в общем файле стилей
   - При необходимости создания нового стиля сначала добавьте его в файл стилей, затем используйте

3. **Альтернативный подход с Tailwind**:
   - Если необходим уникальный стиль для одного компонента, предпочтительнее использовать инлайн Tailwind-классы
   - При использовании Tailwind следуйте единому стилю именования классов во всём проекте
   - Для повторяющихся комбинаций Tailwind-классов создавайте общие стили в файле стилей

4. **Обязательные проверки перед коммитом**:
   - Запустите `npm run type-check` для выявления ошибок типизации
   - Используйте `npm run lint` для обнаружения проблем со стилями и другими аспектами кода
   - Проверьте работу компонента в разных разрешениях и состояниях

5. **Процесс рефакторинга стилей**:
   - Любые изменения в системе стилей должны быть согласованы со всей командой
   - При обновлении общих стилей убедитесь, что все зависимые компоненты продолжают работать корректно
   - Документируйте любые значительные изменения в системе стилей

6. **Импорты и компоненты**:
   - Всегда проверяйте корректность путей импорта
   - Используйте абсолютные импорты для основных компонентов
   - Следите за правильностью структуры директорий при импорте модулей

7. **Процедура ревью кода**:
   - При ревью кода уделяйте особое внимание использованию стилей
   - Проверяйте существование используемых стилей в соответствующих файлах
   - Отклоняйте PR, содержащие ссылки на несуществующие стили или компоненты 