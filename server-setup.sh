...
chmod +x fix-clerk-edge.sh 2>/dev/null || echo "fix-clerk-edge.sh не найден, создаем пустой скрипт"
[ ! -f fix-clerk-edge.sh ] && echo '#!/bin/bash' > fix-clerk-edge.sh && chmod +x fix-clerk-edge.sh
